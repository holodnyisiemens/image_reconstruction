...
def get_next_step_coords(x, y, img_matrix, agent_matrix):
    ...
    # массив распределений в соседних точках матрицы для агентов
    agent_distributions = agent_matrix[neighbor_coords[:, 0], neighbor_coords[:, 1]] / np.sum(agent_matrix)

    dist_diff = img_distributions - agent_distributions

    next_step_coords = neighbor_coords[np.argmax(dist_diff)]
